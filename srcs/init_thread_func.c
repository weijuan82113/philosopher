...
/*   Updated: 2023/03/06 00:11:29 by wchen            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	create_thread(long long p_num, t_philo *philo)
{
	...
	while (i < p_num)
	{
		if (pthread_create((philo[i]).p_thread, NULL, thread_monitor_func,
				(void *)&philo[i]) != 0)
			return (false);
		if (pthread_create((philo[i]).m_thread, NULL, thread_philo_func,
				(void *)&philo[i]) != 0)
			return (false);
		i++;
	}
